	...
	methods ( Access = public )
		...
		% VARARGIN PROCESSING 
			...
			if obj.dimension == 1
				...
				% If a list of points <P> is given.
				if size( varargin{1}, 2 ) == obj.dimension
				...
				else % If axes coordinates are given to generate a grid.
					...
					GridShape = cellfun( @(x) numel(x), varargin( 1 : obj.dimension ) );
			...
			if numel( varargin ) > 0
				...
				assert( mod( numel( varargin ) , 2 ) == 0, ...
					'obj.eval(..., option, value,...) : Pairs of option-value should be provided.');

				tmp = cellfun( @(x) ischar(x) && isrow(x) , varargin(1:2:end) );
				assert( all(tmp), ...
					'obj.eval(..., option, value, ...) : Provided option should be an string.');
				...
				for i = 1 : 2 : numel( varargin )
					...
					switch varargin{i}
						...
						case 'normalized'
							assert( isscalar( varargin{i+1} ) && isboolean( varargin{i+1} ), ...
								'obj.eval(...,''normalized'', value,...) : <value> must be boolean.' );
							bNormalized = varargin{i+1};
							
						case 'interp'
							assert( ischar( varargin{i+1} ) && ischar( varargin{i+1} ), ...
								'obj.eval(...,''interp'',value,...) : <value> must be a valid string.');
							
							tmp = strcmp( varargin{i+1}, {'c0','c1','c2'} );
							assert( any( tmp ), ...
								'obj.eval(...,''interp'',value,...) : <value> should be ''c0'', ''c1'' or ''c2''');
							
							itpMode = varargin{i+1};
							
						otherwise
							error( 'obj.eval(..., option, value, ...) : ''%s'' is not a valid option', varargin{i} );
		...
		% PARAMETER ASSIGNMENT
			switch itpMode
				...
				case 'c1'
					wf = @(x) 1 - x.^2 .* ( 3 - 2 .* x );
				case 'c2'
					wf = @(x) 1 - x.^3 .* ( 10 - 3 .* x .*( 5 - 2 .* x ) );
			end

		% COMPUTATION
			
			% Compute noise for points P
			values = obj.generate( P , wf );
			...
			% Reshaping
			if ~isempty( GridShape )
				values = reshape( values, GridShape );
			end
